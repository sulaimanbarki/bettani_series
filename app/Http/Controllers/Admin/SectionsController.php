...
class SectionsController extends Controller
{
    ...
    public function RecursiveCopySection($section_id, $newSection_id, $book_id)
    {
        $subSections = Section::where('section_id', $section_id)->get();
        if (count($subSections) > 0) {
            foreach ($subSections as $subSection) {

                $new_section = $subSection->replicate()->toArray();
                unset($new_section['id']);
                $new_section['book_id'] = $book_id;
                $new_section['section_id'] = $newSection_id;
                $new_section['slug'] = $subSection->slug . time();
                $new_section['hassection'] = 1;
                $newSection = Section::create($new_section);
                $this->copyUnit($subSection->id, $newSection->id);
                $this->copyImage($subSection->id, $newSection->id);
                $this->RecursiveCopySection($subSection->id, $newSection->id, $book_id);
            }
        } else {
            $this->copyUnit($section_id, $newSection_id);
        }
    }
    ...
    public function copySection(Request $request)
    {
        ...
        $this->copyImage($section_id, $newSection->id);
        $this->RecursiveCopySection($section_id, $newSection->id, $book_id);
    ...
    // Recursive Function 
    public function copyImage($old_section_id, $new_section_id)
    {
        $images = Media::where(['id' => $old_section_id, 'collection_name' => 'sections'])->get();
        foreach ($images as $key => $image) {
            $new_image = $image->replicate()->toArray();
            unset($new_image['id']);
            $new_image->model_id = $new_section_id;
            $newSection = Media::create($new_image);
        }
    }
    ...
    public function getCategoryOption($section)
    {
        ...
        if ($section->Sections()->exists()) {
            ...
            $option .= "<option value='" . $section->id . "'>" . $section->title . " -- (Main)</option>";
