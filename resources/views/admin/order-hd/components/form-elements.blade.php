...
<div class="form-check row" :class="{'has-danger': errors.has('free'), 'has-success': fields.free && fields.free.valid }">
    <div class="ml-md-auto" :class="isFormLocalized ? 'col-md-8' : 'col-md-10'">
        <input class="form-check-input" id="free" type="checkbox" v-model="form.free" v-validate="''" data-vv-name="free" name="free_fake_element">
        <label class="form-check-label" for="free">
            Free or not
        </label>
        <input type="hidden" name="free" :value="form.free">
        <div v-if="errors.has('free')" class="form-control-feedback form-text" v-cloak>@{{ errors.first('free') }}</div>
    </div>
</div>
